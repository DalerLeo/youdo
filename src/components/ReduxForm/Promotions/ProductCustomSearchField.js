...
let priceProductCustomListToken = null

const getOptions = (search, type) => {
    if (priceProductCustomListToken) {
        priceProductCustomListToken.cancel()
    }
    priceProductCustomListToken = CancelToken.source()
    return axios().get(`${PATH.PRODUCT_LIST}?type=${type || ''}&page_size=1000&search=${search || ''}`, {cancelToken: priceProductCustomListToken.token})
        .then(({data}) => {
